module Multigrid_2D_Neumann_BC
  use Exact_Solver
  implicit none
contains
  !---------------------------------------------------------------------------!
  ...
  subroutine MG_Vcycle(Nx,Ny,dx,dy,RHS,U,Level_num,tol,exact_solver,I_cycle)
    ...
    !open(40,file='residual_All_MG4V4.plt')
    ...
    !---------------------------------------------------------------------------!
      do Level=1,Level_num-1
        ...
        do i=0,Level_Nx(Level+1)
           sum = F(Level+1)%data(i,j) + sum
        end do
        end do

        avg = sum/((Level_Nx(Level+1)+1)*(Level_Ny(Level+1)+1))

        do j=0,Level_Ny(Level+1)
        do i=0,Level_Nx(Level+1)
          F(Level+1)%data(i,j) = F(Level+1)%data(i,j) - avg
    ...
    !---------------------------------------------------------------------------!
      ...
      do j=0,Level_Ny(1)
      do i=0,Level_Nx(1)
         sum = F(1)%data(i,j) + sum
      end do
      end do

      avg = sum/((Level_Nx(1)+1)*(Level_Ny(1)+1))

      do j=0,Level_Ny(1)
      do i=0,Level_Nx(1)
        F(1)%data(i,j) = F(1)%data(i,j) - avg
      end do
      end do
  ...
  subroutine Relax(Nx,Ny,dx,dy,F,U)
    ...
    do j=1,Ny-1
    do i=1,Nx-1
      U(i,j) = (1.0d0-w)*U(i,j) + w*(1.0d0/a)*(F(i,j) &
    ...
    ! Neumann B.C
    do i=1,Nx-1
      U(i,0)  = 0.25d0 * (U(i+1,0)+U(i-1,0)+2.0d0*U(i,1)-2.0d0*dx*dx*F(i,0))
      U(i,Ny) = 0.25d0 * (U(i+1,Ny)+U(i-1,Ny)+2.0d0*U(i,Ny-1)-2.0d0*dx*dx*F(i,Ny))
      sum = sum + U(i,0) + U(i,Ny)
    end do

    do j=1,Ny-1
      U(0,j)  = 0.25d0 * (U(0,j+1)+U(0,j-1)+2.0d0*U(1,j)-2.0d0*dx*dx*F(0,j))
      U(Nx,j) = 0.25d0 * (U(Nx,j+1)+U(Nx,j-1)+2.0d0*U(Nx-1,j)-2.0d0*dx*dx*F(Nx,j))
      sum = sum + U(0,j) + U(Nx,j)
    end do

    ! Corner Point
    U(0,0)    = 0.5d0 * (U(1,0)+U(0,1))-dx*dx*F(0,0)
    U(Nx,0)   = 0.5d0 * (U(Nx-1,0)+U(Nx,1))-dx*dx*F(Nx,0)
    U(0,Ny)   = 0.5d0 * (U(0,Ny-1)+U(1,Ny))-dx*dx*F(0,Ny)
    U(Nx,Ny)  = 0.5d0 * (U(Nx-1,Ny)+U(Nx,Ny-1))-dx*dx*F(Nx,Ny)

    sum = sum + U(0,0) + U(Nx,0) + U(0,Ny) + U(Nx,Ny)
  ...
  subroutine Residual(Nx,Ny,dx,dy,F,U,R)
    ...
    do i=1,Nx-1
      R(i,0)  = F(i,0)  - 0.5d0*(U(i+1,0) - 2.0d0*U(i,0) + U(i-1,0))/(dx*dx)    &
                     &  - 0.5d0*(2.0d0*U(i,1) - 2.0d0*U(i,0))/(dy*dy)
      R(i,Ny) = F(i,Ny) - 0.5d0*(U(i+1,Ny) - 2.0d0*U(i,Ny) + U(i-1,Ny))/(dx*dx) &
                     &  - 0.5d0*(-2.0d0*U(i,Ny) + 2.0d0*U(i,Ny-1))/(dy*dy)
    end do

    do j=1,Ny-1
      R(0,j)  = F(0,j)  - 0.5d0*(2.0d0*U(1,j) - 2.0d0*U(0,j)) /(dx*dx)          &
                     &  - 0.5d0*(U(0,j+1) - 2.0d0*U(0,j) + U(0,j-1))/(dy*dy)
      R(Nx,j) = F(Nx,j) - 0.5d0*(-2.0d0*U(Nx,j) + 2.0d0*U(Nx-1,j))/(dx*dx)      &
                     &  - 0.5d0*(U(Nx,j+1) - 2.0d0*U(Nx,j) + U(Nx,j-1))/(dy*dy)
    end do

    ! Corner Point
    R(0,0) = F(0,0) - 0.5d0*(U(1,0) - U(0,0))/(dx*dx) - 0.5d0*(U(0,1) - U(0,0))/(dy*dy)
    R(Nx,0) = F(Nx,0) - 0.5d0*(U(Nx-1,0) - U(Nx,0))/(dx*dx) - 0.5d0*(U(Nx,1) - U(Nx,0))/(dy*dy)
    R(0,Ny) = F(0,Ny) - 0.5d0*(U(1,Ny) - U(0,Ny))/(dx*dx) - 0.5d0*(U(0,Ny-1) - U(0,Ny))/(dy*dy)
    R(Nx,Ny) = F(Nx,Ny) - 0.5d0*(U(Nx-1,Ny) - U(Nx,Ny))/(dx*dx) - 0.5d0*(U(Nx,Ny-1) - U(Nx,Ny))/(dy*dy)
  ...
  subroutine Restriction(Nxf,Nyf,Nxh,Nyh,R,F)
    ...
    !Boundary conditions
    do i=0,Nxh
      F(i,0)   = 1.0d0/16.0d0 * (4.0d0*R(2*i,0)                           &
              &      + 2.0d0 * (R(2*i+1,0)+R(2*i-1,0)+R(2*i,1))           &
              &      + 1.0d0 * (R(2*i+1,1)+R(2*i-1,1)))
      F(i,Nyh) = 1.0d0/16.0d0 * (4.0d0*R(2*i,Nyf)                         &
              &      + 2.0d0 * (R(2*i+1,Nyf)+R(2*i-1,Nyf)+R(2*i,Nyf-1))   &
              &      + 1.0d0 * (R(2*i+1,Nyf-1)+R(2*i-1,Nyf-1)))
    end do

    do j=0,Nyh
      F(0,j)   = 1.0d0/16.0d0 * (4.0d0*R(0,2*j)                           &
              &      + 2.0d0 * (R(1,2*j)+R(0,2*j+1)+R(0,2*j-1))           &
              &      + 1.0d0 * (R(1,2*j+1)+R(1,2*j-1)))
      F(Nxh,j) = 1.0d0/16.0d0 * (4.0d0*R(Nxf,2*j)                         &
              &      + 2.0d0 * (R(Nxf-1,2*j)+R(Nxf,2*j+1)+R(Nxf,2*j-1))   &
              &      + 1.0d0 * (R(Nxf-1,2*j+1)+R(Nxf-1,2*j-1)))
    end do

    ! Corner Point
    F(0,0)     = 1.0d0/16.0d0 * (4.0d0*R(0,0)+ 2.0d0 * (R(1,0)+R(0,1)) + 1.0d0 * R(1,1))
    F(0,Nyh)   = 1.0d0/16.0d0 * (4.0d0*R(0,Nyf)+ 2.0d0 * (R(1,Nyf)+R(0,Nyf-1)) + 1.0d0 * R(1,Nyf-1))
    F(Nxh,0)   = 1.0d0/16.0d0 * (4.0d0*R(Nxf,0)+ 2.0d0 * (R(Nxf-1,0)+R(Nxf,1)) + 1.0d0 * R(Nxf-1,1))
    F(Nxh,Nyh) = 1.0d0/16.0d0 * (4.0d0*R(Nxf,Nyf)+ 2.0d0 * (R(Nxf-1,Nyf)+R(Nxf,Nyf-1)) + 1.0d0 * R(Nxf-1,Nyf-1))
