...
program Poisson_2D
  ...
  real*8 ::dx,dy,tol,rms,x0,xL,y0,yL,pi,start,finish
  ...
  exact_solver = 1  !
  !---------------------------------------------------------------------------!
  !open(20,file='Ans.txt')
  !open(21,file='Exact.txt')
  open(66,file='Result.plt')
  write(66,*)"-------------------------------------------------------"
  ...
  do k=8,11
    ...
    do j=0,ny
    do i=0,nx
    f(i,j) = pi*pi*(cos(pi*x(i))+ cos(pi*y(j)))
    ue(i,j)= -cos(pi*x(i))-cos(pi*y(j))
    end do
    end do
